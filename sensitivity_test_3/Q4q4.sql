...
totals AS (
  SELECT
    COUNT(DISTINCT identifier) AS total_within_sla,
    COUNT(
      DISTINCT CASE
...
SELECT
  ...
  customer_closed,
  agent_closed,
  customer_closed / NULLIF(NULLIF(total_within_sla, 0), 0) AS customer_closed_ratio,
  agent_closed / NULLIF(NULLIF(total_within_sla, 0), 0) AS agent_closed_ratio
FROM
  totals
